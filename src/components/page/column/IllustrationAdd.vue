<template>
  <div>
    <el-form ref="form" :model="comics" label-width="100px" style="width: 500px;" class="upload">
      ...
      <el-form-item label="插画上传" style="width:1000px">
      ...
      <el-form-item>
        ...
        <el-button @click.native.prevent="$router.push('/illustration/list')">取消</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchData() {
      const getAuthor = await api.get('/api/system/author/listAuthor', { perPage: 999 })
      ...
      if (this.$route.params.id) {
        ...
        this.editing = false
        this.comics = { imgKey: '', title: '', type: 2, authorId: '', period: null, comicsList: [], imgList: [] }
      }
    },
    /* eslint-disable */
    ...
    async save() {
      ...
      if (this.editing) {
        ...
        if (code === 200) {
          ...
          this.$router.push('/illustration/list')
        }
      } else {
        this.$set(this.comics, 'imgKey', this.comics.coverKey)
